...
const TableRow = ({ passFromChaildData, passFromParandData, lsgemt }) => {
    ...
    try {
      ...
      if (filterData.length > 0) {
        ...
        if (response.data.qty > filterData[0].quantity) {
          ...
          console.log(" el quantity is low.");
        }
      } else {
        const response = await axios.get(
          `http://127.0.0.1:3001/stok/${barcodeid}`
  ...
  const handleAmound = (price, quantity) => {
  ...
  useEffect(() => {
    try {
      const justAmound =
        sdata && sdata.map((item) => handleAmound(item.price, item.quantity));
      let val = 0;
      for (let i = 0; i < justAmound.length; i++) {
        val = Number(val) + Number(justAmound[i]);
      }
      passFromChaildData(val);
    } catch (error) {
      console.log(error);
    }
  }, [sdata]);
          ...
          sdata.map((stok, index) => {
            return (
              <tr key={index}>
                ...
                <td>{stok.price}</td>
                <td>{stok.discount}</td>
                <td>{handleAmound(stok.price, stok.quantity)}</td>
                <td className="">
                  <button
